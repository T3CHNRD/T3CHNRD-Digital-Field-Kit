...
cmdText = "$ErrorActionPreference='Stop';try{. " & PsLiteral(ui) & " -ToolkitRoot " & PsLiteral(root) & ";exit 0}catch{($_ | Out-String) | Set-Content -LiteralPath " & PsLiteral(logPath) & " -Encoding UTF8;exit 1}"
