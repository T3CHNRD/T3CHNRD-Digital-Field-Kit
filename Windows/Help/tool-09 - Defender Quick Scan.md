...
WHAT IT DOES
Identify registered antivirus products, check Defender availability, and retain scan or skip evidence in text and JSON. Leaves installed protection enabled.

LOCATION
Security > Defender Quick Scan

HOW TO RUN
Open Tools, choose the category above, and click the tool card.
Check the registered antivirus names and Defender status in the output. If Defender is inactive, the scan is skipped; use the installed antivirus and export its scan log. Protection is not disabled. The Antivirus report folder includes a transcript and Antivirus-Status.json.
...
BUNDLED SCRIPT
Windows\Scripts\Security\Invoke-AntivirusQuickScan.ps1
