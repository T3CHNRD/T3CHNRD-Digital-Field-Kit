...
$tools = @(
 [pscustomobject]@{Id='device';Name='Device Information Report';Category='Diagnostics';Description='Collect Windows hardware and OS information.';Path='scripts\windows\System-Info.ps1';Risk='ReadOnly';Ready=$true},
 [pscustomobject]@{Id='network';Name='Network Summary';Category='Network';Description='Review adapters, addressing, DNS and routes.';Path='scripts\windows\Network-Summary.ps1';Risk='ReadOnly';Ready=$true},
 [pscustomobject]@{Id='disk';Name='Disk / Storage Summary';Category='Repair';Description='Review local drive capacity and free space.';Path='scripts\windows\Disk-Summary.ps1';Risk='ReadOnly';Ready=$true},
 [pscustomobject]@{Id='selftest';Name='Embedded Runner Self-Test';Category='System Management';Description='Verify the embedded Run Center and process result handling.';Path='SELFTEST';Risk='ReadOnly';Ready=$true},
 [pscustomobject]@{Id='bsod';Name='BSOD / Crash Report';Category='Diagnostics';Description='Original diagnostic tool pending immutable-script migration.';Path='';Risk='ReadOnly';Ready=$false},
 [pscustomobject]@{Id='console';Name='Launch Diagnostic Consoles';Category='Diagnostics';Description='Original diagnostic tool pending immutable-script migration.';Path='';Risk='ReadOnly';Ready=$false},
 [pscustomobject]@{Id='defender';Name='Defender Audit';Category='Security';Description='Original read-only security tool pending immutable-script migration.';Path='';Risk='ReadOnly';Ready=$false},
 [pscustomobject]@{Id='baseline';Name='Security Baseline Audit';Category='Security';Description='Original read-only security tool pending immutable-script migration.';Path='';Risk='ReadOnly';Ready=$false},
 [pscustomobject]@{Id='repair';Name='Windows Repair Checks';Category='Repair';Description='SFC / DISM workflow pending immutable-script migration.';Path='';Risk='SystemChange';Ready=$false},
 [pscustomobject]@{Id='updates';Name='Update Installed Apps';Category='Optimization';Description='Application update workflow pending migration.';Path='';Risk='SystemChange';Ready=$false},
 [pscustomobject]@{Id='installall';Name='Install All Apps';Category='Deployment';Description='Chrome, Firefox, Malwarebytes, AVG and CCleaner only.';Path='INSTALLALL';Risk='SystemChange';Ready=$true},
 [pscustomobject]@{Id='debloat';Name='Win11Debloat';Category='Deployment';Description='Separate tool only. Never part of Install All.';Path='';Risk='HighRisk';Ready=$false},
 [pscustomobject]@{Id='winutil';Name='WinUtil';Category='Deployment';Description='Separate tool only. Never part of Install All.';Path='';Risk='HighRisk';Ready=$false},
 [pscustomobject]@{Id='setup';Name='Open Setup Resources';Category='Deployment';Description='Setup-resource workflow pending migration.';Path='';Risk='ReadOnly';Ready=$false}
)
...
$brand.Text='v12 Windows field test' + [Environment]::NewLine + 'Original Field Kit UX restored'
