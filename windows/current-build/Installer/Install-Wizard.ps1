...
[CmdletBinding()]
param([Parameter(Mandatory=$true)][string]$SourceRoot,[string]$ReadyPath='')
...
$logPath = Join-Path $env:TEMP 'T3DFK-Install.log'
$script:installSucceeded = $false
$script:launchAfter = $true
$script:installRoot = Join-Path $env:ProgramFiles 'T3DFK'

function Write-InstallLog([string]$Message) {
    ('[{0}] {1}' -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'), $Message) | Out-File -LiteralPath $logPath -Append -Encoding utf8
}
...
function New-Shortcut([string]$Path,[string]$InstallRoot) {
    ...
    $shortcut.IconLocation = '{0},0' -f (Join-Path $InstallRoot 'Toolkit.ico')
    $shortcut.Save()
}
...
function Test-SourcePackage {
    $required = @(
        ...
        'Installer\Install-Wizard.ps1','UNINSTALL-WMDT.vbs'
    )
    ...
    if(-not (Test-Path -LiteralPath (Join-Path $SourceRoot 'Scripts') -PathType Container)) { throw 'Source package is incomplete. Scripts folder is missing.' }
}
...
function Register-Uninstall([string]$Root) {
    ...
    Set-ItemProperty -Path $key -Name DisplayVersion -Value '10.2.4'
    ...
    Set-ItemProperty -Path $key -Name UninstallString -Value ('wscript.exe "{0}"' -f (Join-Path $Root 'UNINSTALL-WMDT.vbs'))
...
$sub.Text='v10.2.4 Windows installer - choose a location, options, then install';$sub.ForeColor=[Drawing.Color]::AliceBlue;$sub.AutoSize=$true;$sub.Location=New-Object Drawing.Point(28,70)
...
$btnInstall.Add_Click({
    try {
        ...
        $startDir=Join-Path $env:ProgramData 'Microsoft\Windows\Start Menu\Programs\T3CHNRD Digital Field Kit'
        New-Item -Path $startDir -ItemType Directory -Force | Out-Null
        New-Shortcut -Path (Join-Path $startDir 'T3CHNRD Digital Field Kit.lnk') -InstallRoot $script:installRoot
        if($chkDesktop.Checked){
            Set-Status 88 'Creating desktop shortcut...'
            New-Shortcut -Path (Join-Path ([Environment]::GetFolderPath('CommonDesktopDirectory')) 'T3CHNRD Digital Field Kit.lnk') -InstallRoot $script:installRoot
        }
        Set-Status 94 'Registering uninstall information...'
        Register-Uninstall -Root $script:installRoot
        @('T3CHNRD Digital Field Kit v10.2.4','Installed: '+(Get-Date),'Source: '+$SourceRoot) | Set-Content -LiteralPath (Join-Path $script:installRoot 'Installed-Version.txt') -Encoding utf8
